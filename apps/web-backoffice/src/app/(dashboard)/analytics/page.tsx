...
const PIE_COLORS = ["#1A1A2E", "#D4AF37", "#2E8B57", "#4169E1", "#FF6347", "#9370DB", "#20B2AA", "#FF8C00"];
              ...
              {analytics.byTicketType.length > 0 ? (
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={analytics.byTicketType}>
                    ...
                    <YAxis tick={{ fontSize: 10 }} allowDecimals={false} />
                    <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                    <Bar dataKey="registered" name="Inscrits" fill="#1A1A2E" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="checkedIn" name="Check-ins" fill="#D4AF37" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <p className="text-sm text-gray-400 py-10 text-center">Aucune donnée</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {analytics.topEvents.map((event) => {
                    const rate = event.registeredCount > 0
