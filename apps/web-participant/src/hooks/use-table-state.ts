...
/**
 * Single source of truth for the URL state of any list / discovery page
 * in the participant app. Mirror of the backoffice hook at
 * `apps/web-backoffice/src/hooks/use-table-state.ts` — identical API
 * surface so the doctrine pattern feels the same in both apps.
 *
 * The doctrine (`docs/design-system/data-listing.md`) mandates that no
 * list page reads `searchParams` or calls `router.push` directly for
 * q/filters/sort/page/pageSize — they all flow through this hook.
 *
 * Step 2 of the doctrine documents the behavioural contract; this file
 * is one of its two sanctioned implementations. If you change the API
 * here, change it in the backoffice mirror in the same commit.
 */
...
export interface UseTableStateOptions<TFilters extends Record<string, unknown>> {
  ...
  debounceMs?: number;
  /**
   * Whether URL mutations are shallow (default: `true`, no Next.js
   * re-render) or full (`false`, triggers a Server Component refresh).
   * Set to `false` for SSR-first pages whose data fetch lives in a
   * Server Component reading `searchParams` — without it, URL changes
   * never reach the server and the rendered page goes stale until a
   * manual refresh. Client-rendered pages using React Query stay on
   * `shallow: true` since the queryKey already reacts to URL changes.
   */
  shallow?: boolean;
}
...
export function useTableState<TFilters extends Record<string, unknown>>(
  ...
  const [{ q }, setQUrl] = useQueryStates(
    ...
    { history: "replace", shallow, throttleMs: 50 },
  );
