...
- Every read path must remember to filter by status. Mitigated by repository-level helpers (`BaseRepository.findActive()`).
...
- `apps/api/src/repositories/base.repository.ts` — `findActive()` helper.
