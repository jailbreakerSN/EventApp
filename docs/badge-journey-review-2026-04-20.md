...
| **Sprint B**                                               |                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3.5 — duplicate scan carries scanned-by identity           | shipped                       | `QrAlreadyUsedError` gains details bag `{ checkedInAt, checkedInBy, checkedInByName, checkedInDeviceId }`; `registrationService.checkIn` returns a tagged `TxOutcome` from the tx and enriches with `userRepository.findById(checkedInBy)` before throwing; backoffice `ScanResultCard` now renders "Validé par Aminata Fall · il y a 12 s" + the device id                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2.1 — collapse badge writers                               | shipped                       | all four writers (`BadgeService.generate`, `BadgeService.bulkGenerate`, `registration.triggers.ts` badge hook, `payment.triggers.ts` badge hook) converge on the deterministic `${eventId}_${userId}` doc id. `generate` wrapped in `db.runTransaction` with create-if-missing + outer fast-path. `bulkGenerate` uses per-item `docRef.create()` via `Promise.allSettled` so a concurrent winner's `pdfURL` / `status` cannot be clobbered (gRPC ALREADY_EXISTS ⇒ silent skip; any other error rethrows). Triggers moved to the same doc id inside their existing transactions. Transaction auditor re-run and clean                                                                                                                                                                                                                   |
| 1.1 + 1.2 — per-event HKDF keys + `kid` rotation           | shipped                       | v4 wire `regId:eventId:userId:nb:na:kid:sig` (7 parts). `signQrPayloadV4` / `deriveEventKey` (HKDF-SHA256(`QR_MASTER`, salt=`eventId`, info=`teranga/qr/v4/${kid}`)) / `generateEventKid`. Verifier made async with `EventKeyResolver`; v1/v2/v3 branches unchanged; fail-closed when resolver omitted. Fresh `qrKid` minted on event create + clone. `POST /v1/events/:eventId/qr-key/rotate` appends the retired kid to `qrKidHistory` so already-issued badges keep verifying. `registration.service.ts` + `payment.service.ts` sign v4 when the event has a kid, v3 fallback otherwise. `checkin.service.ts` + `sponsor.service.ts` + `registration.service.ts checkIn` all pass `resolveEventKeyFromEvent` to the verifier. New optional `QR_MASTER` env var (falls back to `QR_SECRET` during rollout). 806 tests pass (+13 new) |
| **Sprint C**                                               |                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 3.6 — live-scan zone capacity enforcement                  | shipped                       | `RegistrationService.checkIn` throws `ZoneFullError` (409) when a zone is at capacity; counter-key shape matches the bulk path. Fixed a latent reads-after-writes ordering in the tx.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 3.3 c1/5 — `checkins` collection shadow-write              | shipped                       | Per-scan forensic row (success / duplicate / rejected) with device attestation, nonce, reject code, QR version + kid. Live path atomic with the tx; bulk path fire-and-forget during shadow phase. 4 Firestore composite indexes + admin-SDK-only rules.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 3.3 c3/5 — read API                                        | shipped                       | `GET /v1/events/:eventId/checkins` paginated + filterable (`status`, `accessZoneId`, `since`, `until`). Legacy `getHistory` stays on registrations for back-compat.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 4.3 — security anomalies endpoint                          | shipped                       | `GET /v1/events/:eventId/checkin/anomalies` returns duplicates + device mismatches + velocity outliers in one round-trip. Discriminated-union response shape. Gated behind `advancedAnalytics` (Pro+).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 3.3 c2/5 + c4/5 — `checkinLocks` + multi-entry scan policy | shipped                       | `Event.scanPolicy: single \| multi_day \| multi_zone` gates secondary scans via `checkinLocks` (doc id encodes `(registrationId, scope)`). `event.checkedInCount` stays as "unique humans admitted"; `zoneCheckedInCounts` counts every zone entry. Transaction auditor clean on both paths.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 3.3 c4/5 — `setScanPolicy` endpoint                        | shipped                       | `POST /v1/events/:eventId/scan-policy` transactional flip + `event.updated` audit emit with before/after values. No-ops (zero writes, zero emits) when already on target.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Sprint C c5/5 — backoffice widget UI                       | shipped                       | `claude/badge-anomaly-widget-wAhdP` (PR #122) — `AnomalyWidget` in `DashboardTab`, severity-sorted feed, accordion drill-down, teranga-gold upsell for non-`advancedAnalytics` orgs, `paused`-aware polling. Self-review caught & fixed a bug where `anomalyKey` embedded the server-recomputed `detectedAt` and collapsed the expanded row every 10 s.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Sprint D — Polish**                                      |                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2.3 — offline PDF docs + IndexedDB QR fallback             | shipped                       | Code already correct (QR payload cached to IndexedDB by `badge-store.saveBadge()` on every successful fetch; PDF blob cached independently via the Service Worker; page degrades gracefully when either path fails — copy localized in fr/en/wo). Sprint D scope is doc-only: inline contract added on `badge-store.ts` + `badge/page.tsx` so the next maintainer doesn't accidentally couple PDF caching to QR rendering.                                                                                                                                                                                                                                                                                                                                                                                                             |
| 1.4 — don't-store-signature                                | **deferred (documented)**     | Research pass (Sprint D explore agent) established: `Registration.qrCodeValue` is shipped plaintext to staff devices in the offline-sync payload (`checkin.service.ts:118` / `OfflineSyncRegistrationSchema`) and used as the Firestore lookup index for scans (`registrationRepository.findByQrCode`). Removing storage without also reshaping the wire + lookup buys little — a Firestore-backup leak exposes time-bounded signatures (validity window + HKDF key not in the backup), while a staff-device leak is the higher-risk surface and is already mitigated by the Sprint A encrypted envelope. Revisit only if we pick up full payload-decomposition on the Flutter scanner (would also enable client-side HMAC verification of v4 QRs without needing the full signed value pre-shipped).                                  |
| 3.2b — gate photo capture                                  | **deferred (needs UX + CDP)** | Requires a product decision on consent copy (Senegalese CDP — Commission de protection des données personnelles — approval for biometric capture at an event gate) and a scanner-UI designer pass. Not a fraud-blocker: Sprint A's device attestation + Sprint C's security-anomalies widget already surface the screenshot-sharing pattern. Flagged in Wave 9 planning but not scheduled.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Sprint E+                                                  | not started                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
